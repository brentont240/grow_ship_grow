...
func _on_player_has_grown():
	hud.growth = str(player.growth) + "/" + str(growth_limit)
	# Go to the next level
	if player.growth >= growth_limit:
		# Use match to go to the next level using the level variable
		changeLevel()
